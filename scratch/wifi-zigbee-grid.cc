...
int main(int argc, char* argv[]) {
  ...
  mobility.SetPositionAllocator("ns3::GridPositionAllocator", "MinX", DoubleValue(0.0), "MinY", DoubleValue(0.0),
                                "DeltaX", DoubleValue(30.0), "DeltaY", DoubleValue(30.0), "GridWidth",
  ...
  for (auto i = zigbeeStacks.Begin(); i != zigbeeStacks.End(); i++) {
    ...
    zstack->GetNwk()->SetNlmeNetworkFormationConfirmCallback(MakeBoundCallback(&NwkNetworkFormationConfirm, zstack));
    // NLDE-DATA.Indication
    zstack->GetNwk()->SetNldeDataIndicationCallback(MakeBoundCallback(&NwkDataIndication, zstack));
    // NLDE-NETWORK-DISCOVERY.Confirm
    zstack->GetNwk()->SetNlmeNetworkDiscoveryConfirmCallback(MakeBoundCallback(&NwkNetworkDiscoveryConfirm, zstack));
    // NLME-JOIN.Confirm
    zstack->GetNwk()->SetNlmeJoinConfirmCallback(MakeBoundCallback(&NwkJoinConfirm, zstack));
    // NLME-ROUTE-DISCOVERY.Confirm
    zstack->GetNwk()->SetNlmeRouteDiscoveryConfirmCallback(MakeBoundCallback(&NwkRouteDiscoveryConfirm, zstack));
  }
  ...
  for (auto i = zigbeeStacks.Begin(); i != zigbeeStacks.End(); i++) {
    ...
    if (index == 0) {
      ...
      Simulator::ScheduleWithContext(zstack->GetNode()->GetId(), MilliSeconds(index * 500),
                                     &ZigbeeNwk::NlmeNetworkFormationRequest, zstack->GetNwk(), netFormParams);
    } else {
      ...
      Simulator::ScheduleWithContext(zstack->GetNode()->GetId(), Seconds(2 + index * 10),
                                     &ZigbeeNwk::NlmeNetworkDiscoveryRequest, zstack->GetNwk(), netDiscParams);
    }
  }
  ...
  Simulator::Schedule(Seconds(500), &CreateManyToOneRoutes, zigbeeStacks.Get(0), zigbeeStacks.Get(49));
